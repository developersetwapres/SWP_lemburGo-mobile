...
class LocationService {
  Future<DeviceAddress> currentAddress() async {
    if (!await Geolocator.isLocationServiceEnabled()) throw const LocationException('Aktifkan layanan lokasi untuk membuat timestamp otomatis.');
    var permission = await Geolocator.checkPermission();
    if (permission == LocationPermission.denied) permission = await Geolocator.requestPermission();
    if (permission == LocationPermission.denied) throw const LocationException('Izin lokasi belum diberikan. Anda dapat mencoba lagi atau mengatur timestamp manual.');
    if (permission == LocationPermission.deniedForever) throw const LocationException('Izin lokasi ditolak permanen. Buka Pengaturan atau gunakan timestamp manual.');
    try {
    ...
    } catch (_) { return const DeviceAddress(road: 'Lokasi perangkat tidak tersedia', districtCity: '', province: '', isFallback: true); }
  }
  String _first(String? first, [String? second, String? third, String? fourth]) { for (final value in [first, second, third, fourth]) { if (value != null && value.trim().isNotEmpty) return value.trim(); } return ''; }
